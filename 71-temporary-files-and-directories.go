...
needed after the program exits.

Temporary files and directories are useful for this purpose
since they don’t pollute the file system over time.
*/
func learn_temporary_files_and_directories() {
	fmt.Println("\n------------ learn_temporary_files_and_directories --------------")

	/*
		The easiest way to create a temporary file is
		by calling os.CreateTemp.

		It creates a file and opens it for reading and writing.
		We provide "" as the first argument,
		so os.CreateTemp will create the file
	...
	*/
	...
	/*
	   ...
	   The file name starts with the prefix given as the second argument
	   to os.CreateTemp and
	   the rest is chosen automatically to ensure that concurrent calls
	...
	*/
	...
	/*
		...
		itself after some time, but it’s good practice to do this explicitly.
	*/
